...
PYBIND11_MODULE(pcsr, m)
{
    ...
    py::class_<PCSR>(m, "PCSR")
        ...
        .def("add_node", &PCSR::add_node, "Adds a node to the graph")
        .def("add_edge", &PCSR::add_edge, "Adds an edge to the graph with given source node, destination node and edge value", py::arg("src"), py::arg("dest"), py::arg("value"))
        .def("add_edge_update", &PCSR::add_edge_update, "Updates the edge value if the edge exists, else adds the edge to the graph with specified source node, destination node and edge value", py::arg("src"), py::arg("dest"), py::arg("value"))
        ...
        .def("get_n", &PCSR::get_n, "Returns the number of nodes in the graph")
        .def("get_size", &PCSR::get_size, "Returns the memory size taken by the PCSR graph representation")
        .def("print_graph", &PCSR::print_graph, "Prints the graph in adjacency matrix form")
        .def("print_array", &PCSR::print_array, "Prints the CSR arrays of the graph")
